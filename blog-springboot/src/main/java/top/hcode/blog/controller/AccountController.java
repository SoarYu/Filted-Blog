...
public class AccountController {
    ...
    public CommonResult register(@Validated @RequestBody RegisterDto registerDto, HttpServletResponse response){
        ...
        if(!sensitiveFilter.legal(registerDto.getUsername())){
            return CommonResult.errorResponse("注册失败，该用户名含有违禁词！");
        }
        if(user!=null){
        ...
        int insert = userServiceDao.insert(new MUser().setEmail(registerDto.getEmail())
                ...
                .setPassword(SecureUtil.md5(registerDto.getPassword())).setStatus(0));
        if (insert == 1) {
            // 用户可以另一个接口
